...
export const WhyChooseUsCard: React.FC<WhyChooseUsCardProps> = ({ item }) => {
  ...
  return (
    <Card className="group relative h-full flex flex-col justify-between overflow-hidden border border-slate-200/90 bg-white/95 backdrop-blur-md hover:border-[#0072CE]/50 hover:shadow-2xl hover:shadow-[#001738]/10 hover:-translate-y-1.5 transition-all duration-300 rounded-3xl">
      ...
      <div>
        <CardHeader className="p-5 sm:p-7 pb-3 space-y-3 sm:space-y-4">
          <div className="flex items-center justify-between">
            {/* Modern Frosted Icon Capsule */}
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[#0072CE]/10 border border-[#0072CE]/20 flex items-center justify-center text-[#0072CE] group-hover:bg-gradient-to-br group-hover:from-[#001738] group-hover:to-[#0072CE] group-hover:text-white group-hover:border-transparent transition-all duration-300 shadow-xs">
              <Icon className="w-5 h-5 sm:w-6 sm:h-6 stroke-[1.8]" />
            </div>

            {/* Architectural Index Tag */}
            <span className="text-[10px] sm:text-[11px] font-mono font-bold tracking-widest text-[#0072CE] bg-[#F0F9FF] border border-sky-200 px-2.5 sm:px-3 py-1 rounded-full shadow-xs">
              {item.num} // PILLAR
            </span>
          </div>
        ...
        <CardContent className="px-5 sm:px-7 pt-0 pb-3 sm:pb-4">
      ...
      <CardFooter className="px-5 sm:px-7 py-3 border-t border-slate-100 bg-[#F8FAFC]">
